...
class StructuredMesh2D : public Mesh2D {
 ...
 public:
  // Constructors
  StructuredMesh2D(double left, double right, double bottom, double top,
  ...
  inline double getDx() const {
    return (domain_.right_ - domain_.left_) / (nx_ - 1);
  };
  inline double getDy() const {
    return (domain_.top_ - domain_.bottom_) / (ny_ - 1);
  };
  inline double getMeshSize() const override {
    return std::min(getDx(), getDy());
  };
  inline const Domain2D& getDomain() const { return domain_; };
  std::optional<std::size_t> getNodeID(
      std::size_t i, std::size_t j) const;  // from grid indices i and j
  std::optional<std::size_t> getNeighbor(std::size_t,
                                         const std::pair<int, int>&) const;
  inline const std::pair<int, int>& getBoundaryInwardDirection(
      const std::string& tag) const {
    return inward_directions_.at(tag);
  };
