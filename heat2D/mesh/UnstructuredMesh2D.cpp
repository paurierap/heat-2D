...
void UnstructuredMesh2D::meshDomain()
{
    ...
    gmsh::model::mesh::getNodes(nodeTags, coords, paramCoords, -1, -1, false, false);
    ...
    for (const auto& [dim, tag] : physicalGroups)
    {
        ...
        for (std::size_t entityTag : entityTags)
        {
            ...
            gmsh::model::mesh::getNodes(entityNodeTags, entityCoords, entityParamCoords,
                                         dim, entityTag, true, false);

            for (std::size_t nt : entityNodeTags)
            {
                std::size_t localID = tagToLocal.at(nt);

                if (node_to_boundary_node_.find(localID) == node_to_boundary_node_.end())
                {
                ...
                else boundary_nodes_[node_to_boundary_node_.at(localID)].tags_.push_back(name);
    ...
    for (std::size_t t = 0; t < elementTypes.size(); ++t)
    {
        ...
        gmsh::model::mesh::getElementProperties(elementTypes[t], elementName, elemDim,
                                                 order, numNodesPerElement,
...
double UnstructuredMesh2D::getElementArea(std::size_t elementID) const
{
    ...
    return std::abs((p0.x_ * (p1.y_ - p2.y_) + p1.x_ * (p2.y_ - p0.y_) + p2.x_ * (p0.y_ - p1.y_)) / 2.0);
}
