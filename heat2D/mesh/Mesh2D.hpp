...
class Mesh2D
{
    ...
    public:
        ...
        inline const std::unordered_map<std::string, std::vector<std::size_t>>& getBoundaryGroups() const {return boundary_groups_;};
        inline std::vector<std::size_t> getElementNodes(std::size_t elementID) const 
        {
            ...
            for (std::size_t i = start; i < end; ++i) elementNodes.push_back(element_connectivity_[i]);
            return elementNodes;
        };
        ...
        // Other helpers
        inline bool isNodeInner(std::size_t nodeID) const {return node_to_boundary_node_.find(nodeID) == node_to_boundary_node_.end();};
        inline bool isNodeBoundary(std::size_t nodeID) const {return !isNodeInner(nodeID);};
};
