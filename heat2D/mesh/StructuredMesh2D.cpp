...
const std::unordered_map<std::string, std::pair<int, int>>
    StructuredMesh2D::inward_directions_ = {{"Left", {1, 0}},
                                            {"Right", {-1, 0}},
                                            {"Bottom", {0, 1}},
                                            {"Top", {0, -1}}};

StructuredMesh2D::StructuredMesh2D(double left, double right, double bottom,
                                   double top, std::size_t nx, std::size_t ny)
    : Mesh2D() {
  if (left >= right || bottom >= top)
    throw std::invalid_argument(
        "Inconsistent geometrical constraints. The left (or bottom) side "
        "cannot be equal or larger than the right (or top) side.");

  domain_ = {left, right, bottom, top};

  if (nx <= 1 || ny <= 1)
    throw std::invalid_argument(
        "StructuredMesh2D: number of nodes must be positive and greater than "
        "1.");

  nx_ = nx;
  ny_ = ny;

  std::cout << "\nCreating StructuredMesh2D with domain [" << left << ", "
            << right << "] x [" << bottom << ", " << top << "] and " << nx_
            << " x " << ny_ << " nodes...\n";
...
StructuredMesh2D::StructuredMesh2D(const Domain2D& domain, std::size_t nx,
                                   std::size_t ny)
    : StructuredMesh2D(domain.left_, domain.right_, domain.bottom_, domain.top_,
                       nx, ny) {};
  ...
  for (std::size_t i = 0; i < nodes_.size(); ++i) {
    if (nodes_[i].nodeID_ != i) {
      throw std::logic_error(
          "StructuredMesh2D: nodeID_ must match nodes_ index.");
    }
  }
};
...
std::optional<std::size_t> StructuredMesh2D::getNeighbor(
    std::size_t nodeID, const std::pair<int, int>& direction) const {
  int di = direction.first;
  int dj = direction.second;
  int i = nodeID % nx_;
  int j = nodeID / nx_;

  if (i + di < 0 || i + di >= nx_ || j + dj < 0 || j + dj >= ny_)
    return std::nullopt;
  return static_cast<std::size_t>(j + dj) * nx_ +
         static_cast<std::size_t>(i + di);
};

bool StructuredMesh2D::isCorner(std::size_t nodeID) const {
  return nodeID == 0 || nodeID == nx_ - 1 || nodeID == nx_ * (ny_ - 1) ||
         nodeID == ny_ * nx_ - 1;
}
