...
// =============================================================================
double solve_and_get_error(
    HeatPDE2D& solver, const mesh::Mesh2D& mesh,
    std::function<double(double, double, double)> solution, double t_end) {
  Eigen::VectorXd sol = solve_and_get_solution(solver, t_end);
  Eigen::VectorXd exact(sol.size());

  int j = 0;
  for (const auto& node : mesh.getNodes())
    exact[j++] = solution(node.x_, node.y_, t_end);
...
//         discretization)
// =============================================================================
double solve_and_get_error_vs_ref(HeatPDE2D& solver,
                                  const Eigen::Ref<const Eigen::VectorXd>& ref,
                                  double t_end) {
  Eigen::VectorXd sol = solve_and_get_solution(solver, t_end);
...
class DirichletBCTimeConvergence : public testing::Test {
 protected:
  ...
  std::function<double(double, double)> alpha = [](double, double) {
    return alpha_val;
  };

  std::function<double(double, double, double)> zeroBC =
      [](double, double, double) { return 0.0; };

  std::function<double(double, double)> u0 = [](double x, double y) {
  ...
  std::function<double(double, double, double)> exact = [&](double x, double y,
                                                            double t) {
    return std::exp(-2 * M_PI * M_PI * alpha_val * t) * std::sin(M_PI * x) *
           std::sin(M_PI * y);
  };
...
TEST_F(DirichletBCTimeConvergence, ExplicitEuler) {
  ...
  double EE_err_coarse =
      solve_and_get_error_vs_ref(EEsolver_coarse, ref, t_end);
  double EE_err_fine = solve_and_get_error_vs_ref(EEsolver_fine, ref, t_end);

  // Verify Explicit Euler expected convergence rate
  double EE_rate =
      std::log(EE_err_coarse / EE_err_fine) / std::log(dt_coarse / dt_fine);
...
TEST_F(DirichletBCTimeConvergence, ImplicitEuler) {
  ...
  // Solve and compare approximations
  double IE_err_coarse =
      solve_and_get_error(IEsolver_coarse, mesh, exact, t_end);
  double IE_err_fine = solve_and_get_error(IEsolver_fine, mesh, exact, t_end);

  // Verify Implicit Euler expected convergence rate
  double IE_rate =
      std::log(IE_err_coarse / IE_err_fine) / std::log(dt_coarse / dt_fine);
...
TEST_F(DirichletBCTimeConvergence, CrankNicolson) {
  ...
  double CN_err_coarse =
      solve_and_get_error(CNsolver_coarse, mesh, exact, t_end);
  double CN_err_fine = solve_and_get_error(CNsolver_fine, mesh, exact, t_end);

  // Verify Crank Nicolson expected convergence rate
  double CN_rate =
      std::log(CN_err_coarse / CN_err_fine) / std::log(dt_coarse / dt_fine);
...
// * sin(πy), ∂u/∂t = div(α∇u).  Imposing Dirichlet BCs at all sides leads to
// u_left = u_right = u_bottom = u_top = 0, and the initial condition u0 =
// u(x,y,0) = = sin(πx) * sin(πy) + 1/2 * sin(2πx) * sin(πy) is used.
//
...
TEST(HeatPDE2D, CrankNicolsonExpectedError) {
  ...
  auto exact = [&](double x, double y, double t) {
    double mode1 = std::exp(-2 * M_PI * M_PI * alpha_val * t) *
                   std::sin(M_PI * x) * std::sin(M_PI * y);
    double mode2 = 0.5 * std::exp(-5 * M_PI * M_PI * alpha_val * t) *
                   std::sin(2 * M_PI * x) * std::sin(M_PI * y);
    return mode1 + mode2;
  };
...
//
// For u(x,y,t) = exp(-t) * sin(πx) * sin(πy), we have ∂u/∂t = div(α∇u) + f,
// with f(x,y,t) = (-1 + 2π²) * u(x,y,t).  Imposing Dirichlet BCs at all sides
// leads to u_left = u_right = u_bottom = u_top = 0, and the initial condition
// u0 = = u(x,y,0) = sin(πx) * sin(πy) is used.
//
...
TEST(HeatPDE2D, CrankNicolsonWithSource) {
  ...
  // Source term
  auto source = [exact](double x, double y, double t) {
    return (-1.0 + 2.0 * M_PI * M_PI) * exact(x, y, t);
  };
...
TEST_F(DirichletBCTimeConvergence, IntegrateInStages) {
  ...
  EXPECT_NEAR((solver_staged.getSolution() - solver_direct.getSolution())
